...
/*
    ...
    *** run-time data hashing function
    *** ref: https://ru.wikipedia.org/wiki/Murmur2
    *** licence: LGPL
*/

#ifndef __HASH_COMPILE_TIME_H__
#define __HASH_COMPILE_TIME_H__

namespace iroha { namespace ct_hash {
        ...
        class Hasher {
            ...
            template<typename __Type> static constexpr /* h */uint32_t __proc__(__Type& data, uint32_t len, uint32_t offset, uint32_t h, uint32_t m, uint32_t r) {
                return
                    len >= 4 ? __proc__(data, len - 4, offset + 4, __xor__(__mul__(h, m), __mul__(__xor_with_sr__(__mul__(__load__(data, offset), m), r), m)), m, r) :
